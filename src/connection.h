...
enum ConnectionType {
    DetectedConfExternalCommerciumD = 1,
    UISettingsCommerciumD,
    InternalCommerciumD
};
...
struct ConnectionConfig {
    ...
    QString rpcuser;
    QString rpcpassword;
    bool    usingCommerciumConf;
    bool    commerciumDaemon;
    QString commerciumDir;
    QString proxy;
...
private:
    std::shared_ptr<ConnectionConfig> autoDetectCommerciumConf();
    std::shared_ptr<ConnectionConfig> loadFromSettings();

    Connection* makeConnection(std::shared_ptr<ConnectionConfig> config);

    void doAutoConnect(bool tryEcommerciumdStart = true);
    void doManualConnect();

    void createCommerciumConf();
    QString locateCommerciumConfFile();
    QString commerciumConfWritableLocation();
    QString commerciumParamsDir();

    bool verifyParams();
    void downloadParams(std::function<void(void)> cb);
    void doNextDownload(std::function<void(void)> cb);
    bool startEmbeddedCommerciumd();

    void refreshCommerciumdState(Connection* connection, std::function<void(void)> refused);
...
/**
 * Represents a connection to a commerciumd. It may even start a new commerciumd if needed.
