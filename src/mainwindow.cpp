...
{
    ui->setupUi(this);
    logger = new Logger(this, QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)).filePath("cmm-qt-wallet.log"));
    ...
    QObject::connect(ui->actionCheck_for_Updates, &QAction::triggered, [=] () {
        QDesktopServices::openUrl(QUrl("https://github.com/CommerciumBlockchain/cmm-qt-wallet/releases"));
    });
    ...
    // The commerciumd tab is hidden by default, and only later added in if the embedded commerciumd is started
    commerciumdtab = ui->tabWidget->widget(4);
...
void MainWindow::turnstileDoMigration(QString fromAddr) {
    ...
    turnstile.fromBalance->setText(Settings::getCMMUSDDisplayFormat(fnGetAllSproutBalance()));
    for (auto addr : *rpc->getAllZAddresses()) {
    ...
    auto fnUpdateSproutBalance = [=] (QString addr) {
        ...
        auto balTxt = Settings::getCMMUSDDisplayFormat(bal);
        ...
        if (bal < Turnstile::minMigrationAmount) {
            ...
            turnstile.fromBalance->setText(balTxt % " [You need at least " 
                        % Settings::getCMMDisplayFormat(Turnstile::minMigrationAmount)
    ...
    QObject::connect(turnstile.privLevel, QOverload<int>::of(&QComboBox::currentIndexChanged), [=] (auto idx) {
        ...
        turnstile.minerFee->setText(
            Settings::getCMMUSDDisplayFormat(std::get<0>(privOptions[idx]) * Settings::getMinerFee()));
    });
    ...
    QObject::connect(ui->actionSettings, &QAction::triggered, [=]() {
        ...
        QObject::connect(settings.btnClearSaved, &QCheckBox::clicked, [=]() {
            if (QMessageBox::warning(this, "Clear saved history?",
                "Shielded z-Address transactions are stored locally in your wallet, outside commerciumd. You may delete this saved information safely any time for your privacy.\nDo you want to delete the saved shielded transactions now?",
        ...
        if (rpc->getECommerciumD() == nullptr) {
            settings.chkTor->setEnabled(false);
            settings.lblTor->setEnabled(false);
            QString tooltip = tr("Tor configuration is available only when running an embedded commerciumd.");
        ...
        // If values are coming from commercium.conf, then disable all the fields
        auto commerciumConfLocation = Settings::getInstance()->getCommerciumdConfLocation();
        if (!commerciumConfLocation.isEmpty()) {
            settings.confMsg->setText("Settings are being read from \n" + commerciumConfLocation);
            ...
            settings.confMsg->setText("No local commercium.conf found. Please configure connection manually.");
        ...
        if (settingsDialog.exec() == QDialog::Accepted) {
            ...
            if (!isUsingTor && settings.chkTor->isChecked()) {
                ...
                Settings::addToCommerciumConf(commerciumConfLocation, "proxy=127.0.0.1:9050");
                rpc->getConnection()->config->proxy = "proxy=127.0.0.1:9050";

                QMessageBox::information(this, tr("Enable Tor"), 
                    tr("Connection over Tor has been enabled. To use this feature, you need to restart cmm-qt-wallet."), 
                    QMessageBox::Ok);
            }
            if (isUsingTor && !settings.chkTor->isChecked()) {
                // If "use tor" was previously checked and now is unchecked
                Settings::removeFromCommerciumConf(commerciumConfLocation, "proxy");
                rpc->getConnection()->config->proxy.clear();

                QMessageBox::information(this, tr("Disable Tor"),
                    tr("Connection over Tor has been disabled. To fully disconnect from Tor, you need to restart cmm-qt-wallet."),
                    QMessageBox::Ok);
            }
...
void MainWindow::donate() {
    ...
    ui->MemoTxt1->setText(tr("Thanks for supporting cmm-qt-wallet!"));

    ui->statusBar->showMessage(tr("Donate 0.01 ") % Settings::getTokenName() % tr(" to support cmm-qt-wallet"));
...
void MainWindow::postToZBoard() {
    ...
    zb.feeAmount->setText(Settings::getCMMUSDDisplayFormat(Settings::getZboardAmount() + Settings::getMinerFee()));
...
void MainWindow::importPrivKey() {
    ...
    pui.helpLbl->setText(QString() %
                        ...
                        tr("The keys will be imported into your connected commerciumd node"));  
...
void MainWindow::exportTransactions() {
    ...
    QString exportName = "commercium-transactions-" + QDateTime::currentDateTime().toString("yyyyMMdd") + ".csv";
 ...
 * This might fail for various reasons - Remote commerciumd, non-standard locations, custom params passed to commerciumd, many others
*/
void MainWindow::backupWalletDat() {
    if (!rpc->getConnection())
        return;

    QDir commerciumdir(rpc->getConnection()->config->commerciumDir);
    QString backupDefaultName = "commercium-wallet-backup-" + QDateTime::currentDateTime().toString("yyyyMMdd") + ".dat";

    if (Settings::getInstance()->isTestnet()) {
        commerciumdir.cd("testnet3");
        backupDefaultName = "tesetnet-" + backupDefaultName;
    }
    
    QFile wallet(commerciumdir.filePath("wallet.dat"));
    if (!wallet.exists()) {
        QMessageBox::critical(this, tr("No wallet.dat"), tr("Couldn't find the wallet.dat on this computer") + "\n" +
            tr("You need to back it up from the machine commerciumd is running on"), QMessageBox::Ok);
        return;
    }
...
void MainWindow::exportKeys(QString addr) {
    ...
    QObject::connect(pui.buttonBox->button(QDialogButtonBox::Save), &QPushButton::clicked, [=] () {
        QString fileName = QFileDialog::getSaveFileName(this, tr("Save File"),
                           allKeys ? "commercium-all-privatekeys.txt" : "commercium-privatekey.txt");
        QFile file(fileName);
...
void MainWindow::setupCommerciumdTab() {    
    ui->commerciumdlogo->setBasePixmap(QPixmap(":/img/res/commerciumdlogo.gif"));
}
...
void MainWindow::setupRecieveTab() {
    ...
    QObject::connect(ui->listRecieveAddresses, 
        ...
        ui->rcvBal->setText(Settings::getCMMUSDDisplayFormat(rpc->getAllBalances()->value(addr)));
