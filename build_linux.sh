...
cd ${project_dir}/build/linux/gcc/x86_64/release/CommerciumQT/
...
# Copy CommerciumQT's Qt dependencies, including QML dependencies if your
...
cp -R * "${project_dir}/installer/linux/packages/com.CommerciumQTproject.CommerciumQT/data/"

# Copy files to a portable archive inside the containing folder
# created earlier
echo "Packaging portable archive..."
cd ..
7z a CommerciumQT_${TAG_NAME}_linux_x86_64_portable.zip CommerciumQT

# Use the Qt Installer Framework to create an installer executable
echo "Creating installer..."
cd "${project_dir}/installer/linux/"
binarycreator --offline-only -c config/config.xml -p packages CommerciumQT_${TAG_NAME}_linux_x86_64_installer
